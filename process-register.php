<?php
        ...
        && isset($_POST['password1']) && isset($_POST['password2']))
    {
        ...
        {
            header("Location: register.php?error=" . urlencode("ชื่อผู้ใช้ต้องมีความยาวอย่างน้อย 8 ตัวอักษร"));
            die();
        }
        */
        if(empty($email))
        ...
        {
            $query_email_account = "SELECT email FROM customer WHERE email = '$email'";
            ...
            else
            {
                ...
                $call_create_account = mysqli_query($connect, $query_create_account);
                if($call_create_account)
                {
